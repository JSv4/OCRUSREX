...
from OCRUSREX.ocrusrex import OCRPDF, Multithreaded_OCRPDF, Multiprocessed_OCRPDF
import os, tika, time
tika.initVM()
...
### TEST SETTINGS
remove_test_output = True #If you want the test OCRed docs to be preserved, change this to False
...
start = time.time()
file_obj = Multiprocessed_OCRPDF(source=source_file)
...
Multiprocessed_OCRPDF(source=source_file, targetPath=target_file)
end = time.time()
print("\t Started: {0} | Ended: {1}. ELAPSED: {2} ({3} per page)".format(start, end, end-start, (end-start)/page_count))
print("\t Levenshtein Distance (~Accuracy): {0}".format(Levenshtein.ratio(extractPdfText(target_file), benchmark_text)*100))
if remove_test_output:
...
with open(source_file,'rb') as s:
	file_obj = Multiprocessed_OCRPDF(source=s.read())
	with open(target_file, "wb+") as f:
		f.write(file_obj)
end = time.time()
print("\t Started: {0} | Ended: {1}. ELAPSED: {2} ({3} per page)".format(start, end, end-start, (end-start)/page_count))
...
start = time.time()
target_file=os.path.join(dirname, "Test10.pdf")
with open(source_file,'rb') as s:
...
target_file = os.path.join(dirname, "Test11.pdf")
with open(target_file, "wb+") as f:
    f.write(file_obj)
end = time.time()
print("\t Started: {0} | Ended: {1}. ELAPSED: {2} ({3} per page)".format(start, end, end-start, (end-start)/page_count))
...
start = time.time()
target_file=os.path.join(dirname, 'Test12.pdf')
